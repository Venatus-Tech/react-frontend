...
const db = firestore.collection("gcl_valo_web");
...
document.getElementById("valoform").addEventListener("submit", submitForm);
...
const uploadImage = async () => {
  ...
  let imageurl;
  const task = ref.child(`gcl_valo.web/${name}`).put(file, metaData);
  await task
    .then((snapshot) => {
